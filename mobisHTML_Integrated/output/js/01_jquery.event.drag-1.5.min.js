...
*/
(function(E) {
    E.fn.drag = function(L, K, J) { if (K) { this.bind("dragstart", L) } if (J) { this.bind("dragend", J) } return !L ? this.trigger("drag") : this.bind("drag", K ? K : L) };
    var A = E.event,
        B = A.special,
        F = B.drag = {
            not: ":input",
            ...
            teardown: function() {
                ...
                if (this === F.dragging) { F.dragging = F.proxy = false }
                G(this, true);
    ...
    function H(L) {
        ...
        } else { if (F.dragging || (M.which > 0 && L.which != M.which) || E(L.target).is(M.not)) { return } }
        switch (L.type) {
            case "mousedown":
