        ...
        messageBox: function(type, message, properties, input) {
            $(this).chain(function() {
                ...
                if (type == 'alert' || type == 'info' || type == 'error') {
                    ...
                    $('#BoxAlertBtnOk, #BoxAlertBtnHome').val(properties.textBoxBtnOk).css({ 'border': '0px', 'background-color': 'transparent', 'color': '#00aad2' });
...
var first_url00 = location.href;
var result = first_url00.indexOf("?kind=app");
(function($) {

    $.fn.swipe_slider = function(settings) {
        ...
        var opts = [];
        opts = $.extend({}, $.fn.swipe_slider.defaults, settings);

        return this.each(function() {

            var _this = this;

            $.fn.extend(this, swipe_slider);
            this.opts = opts;
            this.init();
    ...
    var swipe_slider = {
        ...
        init: function() {
            ...
            this._tg = $(this);
            //this._list = this._tg.children(".Heading2");
            ...
            var b_area = parseInt($(".btn_area").css("width"));
            var c_width = b_area - 105;
            $("#count").css({ "width": c_width + "px" });

            //window.innerWidth <= 640 ? $(this).children().append("<footer><div class='footer_wrap'><div class='footer_img'></div><p><span class='line1'></span> <span class='line2'></span></p></div></footer>") : true;

            $(this).children().append("<footer><div class='footer_wrap'><div class='footer_img'></div><p><span class='line1'></span> <span class='line2'></span></p></div></footer>");
            ...
            if (this.opts.btn_prev && this.opts.btn_next) {
                ...
                this.opts.btn_prev.unbind("click").bind("click", function() {
                    ...
                    var ff1 = ff[1];
                    if (ff0 == "1") {
                        // #해시값이 있는 경우, TOC 클릭해서 들어온 경우 UM동작 이전페이지 링크
                        if (result == -1) {

                            // 앞 주소가 있다면
                            if ($("div#wrapper").attr("data-prev")) {

                                // 가로, 세로 페이지 기억을 위한 쿠키 생성 (핸드북용)
                                $.cookie('this_page', null);
                                last_h2 = $("#prev_page #root").children("div[class*='Heading2']").length - 1;
                                $.cookie('this_page', 'last');
                                //window.location.href= $("div#wrapper").attr("data-prev") + "#";
                                //20180920
                                //window.location.href = $("div#wrapper").attr("data-prev") + "#" + last_h2;
                                window.location.href = $("div#wrapper").attr("data-prev");
                            }
                            // 앞 주소가 없으면
                            else {
                        ...
                        //url id 참조 수정 20180920
                        /*
                                                var chapterObj = chapterId.findIndex((item, idx) => {
                                                    return item.chapter_id === ref_url[1];
                                                });

                        var chapterObj = chapterId.findIndex(function(item, idx) {
                            return item.chapter_id == ref_url[1];
                        });*/
                        ...
                        for (var i = 0; i < chapterId.length; ++i) {
                            if (chapterId[i].chapter_id === ref_url[1] && chapterId[i].url === fileName) {
                                chapterObj = i;
                                break;
                            }
                        }

                        h2 = chapterId[parseInt(chapterObj) - 1].chapter_id;

                        //h2 = parseInt(ref_url[1]) - 1;
                        window.history.pushState(null, "", ref_url[0] + "#" + h2)
                    }
                    return false;
                })
                this.opts.btn_next.unbind("click").bind("click", function() {
                    ...
                    if (ff0 == ff1) {
                        ...
                        if (result == -1) {
                            // 앞 주소가 있다면
                            if ($("div#wrapper").attr("data-next")) {
                                // 가로, 세로 페이지 기억을 위한 쿠키 생성 (핸드북용)
                                $.cookie('this_page', null);
                                //20180920
                                //window.location.href = $("div#wrapper").attr("data-next") + "#0";
                                window.location.href = $("div#wrapper").attr("data-next");
                            }
                            // 앞 주소가 없으면
                            else {
                                return ((this._start[0] == 0 && this._left > 0) || (this._start[last_p] == 0 && this._left < 0));
        ...
        touchend: function(e) {
            ...
            if ((e.type == "touchend" && e.originalEvent.touches.length <= 1) || e.type == "dragend") {
                ...
                // console.log(ref_url)
                var url_no = parseInt(chapterObj[0].h2_no);

                last_2 = $("#wrapper > #root").children("div[class*='Heading2']").length - 1;

                if ((url_no != last_2 && this.direction() == -1) || (this.direction() == 1 && url_no != 0)) {
                    //alert(ref_url[1]+"=="+last_2+"=="+h2+"========="+this.direction());
                    h2 = url_no - (this.direction()); //이전 페이지 id
                    /*
                                        var chapterObj2 = chapterId.findIndex((item, idx) => {
                                            return item.chapter_id === ref_url[1];
                                        });


                    var chapterObj2 = chapterId.findIndex(function(item, idx) {
                        return item.chapter_id == ref_url[1];
                    });*/
                    ...
                    h2_id = chapterId[parseInt(chapterObj2) - (this.direction())].chapter_id;
                    // 해시가 없는 경우 주소에서 #빼기
                    var sharp = "";
                    if (h2_id == "") {
                        sharp = ""
                    ...
                    // if (!isNaN(h2) && h2 >= 0 && h2 <= last_2) {
                    window.history.pushState(null, "", ref_url[0] + sharp + h2_id);
                    // }
                }
                ...
                //마지막 페이지 터치 완료 했을 경우, 내부 그림자 제거
                $("#wrapper").css("box-shadow", "0px 0px 0px #085A96 inset");
        ...
        position: function(d) {
            ...
            if (this.opts.roll) {
                ...
                //console.log(Math.abs(this._left)+"=="+this._range);
                if (Math.abs(this._left) > this._range) {
        ...
        limit_chk: function() {
            ...
            if (result == -1) {
                ...
                var last_p = parseInt((this._len - 1) / this._view) * this._view;
                //return ( (this._start[0] == 0 && this._left > 0) || (this._start[last_p] == 0 && this._left < 0) );
                if ((this._start[0] == 0 && this._left > 0)) {

                    if ($("div#wrapper").attr("data-prev")) {

                        // 가로, 세로 페이지 기억을 위한 쿠키 생성 (핸드북용)
                        $.cookie('this_page', null);
                        last_h2 = $("#prev_page #root").children("div[class*='Heading2']").length - 1; //last h2 no
                        ...
                        $("#wrapper").css("box-shadow", "-2px -10px 25px #005390 inset");
                        return ((this._start[0] == 0 && this._left > 0) || (this._start[last_p] == 0 && this._left < 0));
                    }
                ...
                } else if (this._start[last_p] == 0 && this._left < 0) {
        ...
        go_page: function(i, e) {
            ...
            if ($.cookie('click_page') != 0) {
                $("#root > div[class*='Heading2']").animate({ scrollTop: 0 }, 1);
            }
        },
