...
$.extend($.expr[":"], {
    "containsNoCase": function(elem, i, match, array) {
        match_txt = match[3].replace(/(\s*)/g, "");
        match_txt2 = new RegExp(match_txt, 'gi');
        return (elem.textContent || elem.innerText || "").replace(/(\s*)/g, "").match(match_txt2) != null;
    }
});
...
$(document).ready(function() {
    ...
    var search_short = message[language].search_short;  // "검색어가 너무 짧습니다."
    ...
    // search.html -> #no_result Height 구하기    
    // $("#no_results").css("height", $(window).height() - 390);
    // if ($(window).width() <= 640) {
    //     $("#no_results").css("height", $(window).height() - 390);
    // }
    ...
    /////////// 연관검색어
    $('#id_searchwrap').append('<div class="ul_box"><ul></ul></div>');
    ...
    $('#id_search').keyup(function(e){
        ...
        // li 사이즈 잡기
        var input_width = $('#id_search').innerWidth();
        var input_height = $('#id_search').innerHeight();
        $('.ul_box').css({'width': input_width, 'height' : (input_height * 5) + 1 + 'px'})
        $('.ul_box > ul > li').css({'height': input_height,'line-height': input_height + 'px'})

        // li갯수 5개 이상일 때만 스크롤 보이게 하기
        if($('.ul_box > ul > li').length <= 5){
            $('.ul_box').css({'overflow' : 'hidden', 'border' : 'none'})
            $('.ul_box > ul > li:last-child').css({'border-bottom':'1px solid #bec2cc'})
            $('.ul_box').css({'height' : (input_height * $('.ul_box > ul > li').length) + 1 + 'px', 'border-left':'1px solid #bec2cc', 'border-right':'1px solid #bec2cc'})
        }
        else if($('.ul_box > ul > li').length > 5){
            $('.ul_box').css({'overflow-y' : 'scroll', 'border':'1px solid #bec2cc'});
        }
    ...
    // #id_results2 min-height
    var searchW = $('#search_wrap').outerHeight(true)
    var searchT = Number($('#search_top').css('margin-bottom').split('px')[0])
    var fooH = $('footer').outerHeight()
    var results2B = Number($('#id_results2').css('margin-bottom').split('px')[0])
    $("#id_results2").css({'min-height': $(window).outerHeight() - (searchT + searchW + fooH + results2B)})
    ...
    function init() { //검색 새롭게 할 경우 초기 실행 할 것.
        ...
        search_tag = ""; //태그 초기화
        //search_tag2 = ""; //태그 초기화
        // last_chap = parseInt(search[search_len - 1].chapter_i) + 1;
        last_chap = parseInt(search[search_len - 1].chapter_i) + 1;
        ...
        //		search_tag2 = search_tag2.replace(/\뺚apos;/g, '&apos;');
        //
        ...
        for (var i = 0; i < search_len; i++) {
            ...
            // search_tag += "<div id='where_chap'>" + search[i].chapter_i + "</div>";
        ...
        search_tag = search_tag.replace(/\뺚apos;/g, '&apos;');

        $("#id_json_results").html(search_tag); //#id_json_results로 삽입
    ...
    $("#id_search_button").live("click", function() {
        ...
        if ($("#id_search").val().length == 0 && search_url == -1) {
            ...
            setTimeout(function() {
                ...
                csscody.error("<p>" + search_short + "<a ref=''></a></p>");  // 검색어가 너무 짧습니다. 
            }, 50);
    ...
    if (mainKeyword != null) {
        ...
        $.cookie("txt_search", null, { path: '/', expires: -1 });
    }
    ...
    function search_keyword_toc() {
        ...
        var user_keyword_lowcase = $("#id_search").val().replace(/\İ/g, "햒"); //입력창 검색을 위한 소문자 변환 값
        //괄호 검색을 위한 로직	1	
        ...
        $("#id_json_results #root").each(function() {
            ...
            $(this).find("#topic_body:containsNoCase('" + user_keyword_lowcase + "')").each(function() {
                ...
                if ($(this).html().toLowerCase().substring(idx1, idx1 + txt_len) == user_keyword_lowcase) {
                    ...
                    var substr2 = $(this).html().substring(idx1, idx1 + 50 + txt_len) + "...";
                    ...
                    var substr2 = $(this).html().substring(search_txt.index, search_txt.index + 53 + txt_len) + "...";
                    var sum_text = substr1 + substr2;
                }
                if (init_num == 0) {
                    for (var key = 1; key < last_chap; key++) {
        ...
        //옵션2. 종류별 구분 로직 시작*****************************************************//	
        //4-1. #id_json_results_kind에 초기 챕터별 방을 생성
        t_html = "";
        t_html = t_html + "<div id='search_box'><div id='search_kind1'><div id='chapter1'><div id='title'></div></div></div>"
        t_html = t_html + "<div id='title_line'></div>"
        t_html = t_html + "<div class='custom-select'><div class='wrap_ul'><ul id='sel_chap'></ul></div></div><div id='search_kind1'><div id='view2'></div></div></div>";  // select 생성
        ...
        setTimeout(function() {
            $("#id_results2 #topic_h1").each(function() {
                var new_html = $(this).html().replace(search_regex2, to_color);
                new_html = new_html.replace(search_regex3, to_color).replace(/혂/g, "<span class='red'>").replace(/휶/g, "</span>").replace(/꺆/g, "\(").replace(/냒/g, "\)").replace(/땪/g, "\+").replace(/먂/g, "\\").replace(/뺚/g, "\&").replace(/샦/g, "\[").replace(/앾/g, "\]").replace(/쟊/g, "\?").replace(/챢/g, "\*").replace(/캮/g, "\.").replace(/햒/g, "\İ");
        ...
        // 1) select에 목록 나타내기
        $("#id_results2 div[id*='chapter']").each(function() {
            if($(this).children('#package').size() >= 1){  // 검색결과 1개 이상일 때
                var s_html = s2_html = s_cnt = "";
                for(var key = 1; key < $(this).length; key++){
                    s_html += "<li class='chapter" + key + "'>" + $(this).children('#ch').text() + " (" + $("#id_results2 #chapter" + key + " #package").length + ")</li>"
                    // 총 검색 결과 개수
                    s_cnt = s_cnt + $("#id_results2 #chapter" + key + " #package").length;
                }
                s2_html = s2_html + "<li class='0'>" + message[language].all_text + " [" + s_cnt + "]</li>";  //all
        ...
        // 2) select에 목록 나타내기
        var s_html = s2_html = s_cnt = "";
        // s_html = s_html + "<option value='chapter"+last_chap+"'>"+hb_tit+" ("+$("#id_results2 #chapter"+last_chap+" #package").length+")</option>";
        s_cnt = $("#id_results2 #chapter" + last_chap + " #package").length; // h2갯수
        for (var key = 1; key < last_chap; key++) {
            s_html += "<li class='chapter" + key + "'>" + eval("w" + key).replace(/꺆/g, "\(").replace(/냒/g, "\)").replace(/땪/g, "\+").replace(/먂/g, "\\").replace(/뺚/g, "\&").replace(/샦/g, "\[").replace(/앾/g, "\]").replace(/쟊/g, "\?").replace(/챢/g, "\*").replace(/캮/g, "\.").replace(/햒/g, "\İ") + " [" + $("#id_results2 #chapter" + key + " #package").length + "]</option>"
                //총 검색 결과 개수
            s_cnt = s_cnt + $("#id_results2 #chapter" + key + " #package").length;
        }
        p_html = "<p>" + message[language].all_text + " [" + s_cnt + "]</p>";
        s2_html = s2_html + "<li class='0'>" + message[language].all_text + " [" + s_cnt + "]</li>";
        $('.custom-select').prepend(p_html)
        $("#sel_chap").append(s2_html);  // 옵션 All
        $("#sel_chap").append(s_html);   // 옵션목록



        // 3) 옵션 갯수 0개이면 안보이게 처리
        $(".custom-select #sel_chap").children().each(function() {
            let target = $(this).text().split("[")
        ...
        $('.custom-select').click(function(){
            ...
            // li click event
            $('#sel_chap li').click(function(){
                var li_text = $(this).text()
                var opt_val = $(this).attr('class')
                $('#search_kind1 > #view2 > div').each(function(){
                    var c_num = $(this).attr('id')
                    // All 클릭시
                    if(opt_val == 0){
                        $('.custom-select').children('p').text($('#sel_chap li:first-child').text())  // 클릭했을 때 all로 되돌아오도록
                        $('#search_kind1 > #view2 > div').each(function(){
                    ...
                    // 선택한 결과 확인하기
                    else if(opt_val == c_num){
                        $(this).css('display', 'block')
                        $('.custom-select').children('p').text(li_text) // 클릭했을 때 해당 li가 보이도록
                    }
                    else if(opt_val != c_num){
                        $(this).css('display', 'none')
                    }
        ...
        // 220823 검색결과 아무것도 없을 때!
        if ($("#package").size() == 0) {
            $('#title_line').css({'display':'none'})
            // $('#search_box').append('<div class="noRes"><span class="no_results_en">NOT FOUND!</span><br><span class="no_results_text">' + message[language].result + '</span></div>')
            $('.custom-select').css({'display': 'none'})
            $("#no_results, #no_results > div").fadeIn(600);
            $("#id_results2").fadeOut(0);
        }
        ...
        if (search_pos == "find" && $("div[id*='chapter']").length == result_txt) {
            csscody.alert("<p>" + find_err + "<a ref=''></a></p>");  // '검색어가 너무 짧습니다' 안내창
        }
        //4-8. 검색어 처리 영역
        ...
        /*$("#id_results2").css("height",$(document).height()-310);
        $("#search_box").css("height",$(document).height()-310);*/
        // $("#id_results2").fadeIn(600);
        // $("#no_results").fadeOut(0);
    ...
    function insert_result_kind(heading, sdom) {
        ...
        if (heading == "topic_h1") {
            ...
            if (sdom.next().html() == "" || sdom.next().html() == " ") {
                $("#id_json_results_kind #chapter1").append("<div id='package'><a href='../" + topic_link.capitalize().replace(/캮/g, "\.") + "' id='result_link'></a></div>"); //package 빈방 생성
    ...
    function insert_result_chapter(chapter_num, sdom, sum_text) { //sdom : id_json_results/root/내부 div들		
        ...
        $("#id_json_results_chapter #chapter" + chapter_num).append("<div id='package'><a href='../" + topic_link.capitalize().replace(/캮/g, "\.") + "' id='result_link'></a></div>"); //package 빈방 생성
    ...
    //**********************************************************************************************************************************************************************************************************//		
    //최근검색어 로직	
    //$("#recent_area, #id_results2").css("width", $(window).width()-20); //입력창과 너비 맞춤
        ...
        $("#id_search").focus(function() {
            $('#recent_keywords li').each(function() {
                if ($(this).children('#history_delete').size() == 0) {
                    $(this).prepend("<div id='history_delete'><img src='../images/del_bt.png' alt='' /></div>");
    ...
    $("#recent_area").append("<h3 id='recent_area_close'><img src='../images/close_bt2.png' /> " + search_close + "</h3>");
    ...
    $("#result_link").live('click', function() {
        var result_value = "";
        $(this).children("div").children(".red").each(function(j) {
            if (j == 0) {
        ...
        if (result_value != null && result_value != undefined) {
            $.cookie("txt_search", result_value, { path: '/' });
        }
    });
