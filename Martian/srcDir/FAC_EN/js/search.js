...
$(document).ready(function() {
    ...
    //UI text
    var language = $("html").attr("data-language").toUpperCase(); //페이지 언어값 추출
    ...
    function init() { //검색 새롭게 할 경우 초기 실행 할 것.
        ...
        for (var i = 0; i < search_len2; i++) {
            ...
            search_tag2 += "<div id='topic_link'>" + "content" + search2[i].chapter_i + ".html" + search2[i].toc_id + "</div>";
            search_tag2 += "</div>";
        }
        ...
        for (var i = 0; i < search_len; i++) {
            ...
            search_tag += "<div id='topic_link'>" + "content" + search[i].chapter_i + ".html" + search[i].toc_id + "</div>";
            search_tag += "</div>";
        }
    ...
    function search_keyword_toc() {
        ...
        t_html = t_html + "<div id='search_box'><div id='search_kind1'><div id='chapter1'><div id='title'></div></div></div><div id='search_kind1'><div id='view2'></div></div></div>";
        ...
        if (search_pos == "find" && $("div[id*='chapter']").length == result_txt || $(".package").length == 0) {
        ...
        if (search_cnt < 1) {
            if ($("#recent_keywords li").length < 5) {
    ...
    function insert_result_kind(heading, sdom) {
        ...
        if (heading == "topic_h1") {
            ...
            if (sdom.next().html() == "" || sdom.next().html() == " ") {
                $("#id_json_results_kind #chapter1").append("<div class='package'><a href='../" + topic_link.capitalize().replace(/캮/g, "\.") + "'></a></div>"); //package 빈방 생성
    ...
    function insert_result_chapter(chapter_num, sdom, sum_text) { //sdom : id_json_results/root/내부 div들		
        ...
        $("#id_json_results_chapter #chapter" + chapter_num).append("<div class='package'><a href='../" + topic_link.capitalize().replace(/캮/g, "\.") + "'></a></div>"); //package 빈방 생성
    ...
    //검색결과 switch
    $("#switch_view div").on("click", function() {
    ...
    $(window).on("orientationchange", function() {
        $("#recent_area, #id_results2").css("width", $(window).width() - 20);
    })
        ...
        $("#id_search").focus(function() {
            $('#recent_keywords li').each(function() {
                if ($(this).children('#history_delete').size() == 0) {
                    $(this).prepend("<div id='history_delete'><img src='../contents/images/template/del_bt.png' alt='' /></div>");
    ...
    $("#history_delete").on('click', function() {
    ...
    //최근검색어 클릭 시, 검색 동작
    $('#recent_keywords li').on('click', function() {
    ...
    $("#recent_area").append("<h3 id='recent_area_close'><img src='../contents/images/template/close_bt2.png' /> " + search_close + "</h3>");
