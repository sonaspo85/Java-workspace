...
$(document).ready(function() {
    $("b" + "od" + "y").append('<d' + 'iv i' + 'd="id_' + 'js' + 'o' + 'n_res' + 'ults" style="di' + 'splay:no' + 'ne;"></d' + 'iv> <di' + 'v id="id_' + 'js' + 'on_' + 'resu' + 'lts' + '_cha' + 'pter" cl' + 'ass="b' + '" style="dis' + 'play:none;"></div><div id="id_' + 'j' + 'so' + 'n_resul' + 'ts_' + 'ki' + 'n' + 'd" class="' + 'v" sty' + 'le="' + 'displa' + 'y:non' + 'e;"></div>'); /*if($(document).height()==400){Math.max(0)}*/
});

$(document).ready(function() {
    csscody.initialize();
    var language = $("html").attr("data-language"); //페이지 언어값 추출
    close_msg = message[language].close;
    if (language == "Japanese") {
        $("#home_txt").css({ "top": "32px" });
    }
});
        ...
        messageBox: function(type, message, properties, input) {
            $(this).chain(function() {
                ...
                if (type == 'alert' || type == 'info' || type == 'error') {
                    if (type == 'alert') {
                        $('#' + this.options.name + '-Buttons').append('<input id="BoxAlertBtnHome" type="submit" />');
                    } else {
                        $('#' + this.options.name + '-Buttons').append('<input id="BoxAlertBtnOk" type="submit" />');
                    }
                    $('#BoxAlertBtnOk, #BoxAlertBtnHome').val(properties.textBoxBtnOk).css({ 'border': '0px', 'background-color': 'transparent', 'padding': '2px 5px', 'color': '#6090ff', "font-weight": "bold" });
                    ...
                    $('#BoxAlertBtnHome').bind('click', $.bind(this, function() {
                        this.options.onReturn = true;
