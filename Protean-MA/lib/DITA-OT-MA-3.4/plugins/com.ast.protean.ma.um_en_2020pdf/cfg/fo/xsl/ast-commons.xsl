    ...
    <xsl:template match="*" mode="commonTopicProcessing">
    	...
    	<xsl:if test="not(ancestor::*[contains(@class, ' map/map ')]/opentopic:map//*[contains(@class, ' map/topicref ')][@id=$id]/@product='html')">
    		...
    		<fo:block>
    			...
    			<xsl:apply-templates select="* except(*[contains(@class, ' topic/title ') or
                ...
                <xsl:apply-templates select="*[contains(@class,' topic/topic ')][not(matches(@outputclass, 'backcover'))]" />
