<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="*[contains(@class,' topic/topic ')]/*[contains(@class,' topic/title ')]">
        <xsl:choose>
            ...
            <xsl:when test="parent::*/@outputclass[matches(., 'pagebreak')]">
    ...
    <xsl:template match="*" mode="processTopicTitle">
        ...
        <fo:block>
            ...
            <fo:block>
                ...
                <xsl:if test="$cnt = 1">
                    <xsl:attribute name="axf:baseline-grid">new</xsl:attribute>
                    <xsl:attribute name="axf:baseline-block-snap">before</xsl:attribute>
                </xsl:if>
    ...
    <xsl:template match="*[contains(@class,' topic/image ')]" name="image">
        ...
        <xsl:choose>
            ...
            <xsl:when test="@placement = 'break'">
                <xsl:if test="parent::*/preceding-sibling::node()[1][self::processing-instruction('pagebreak')]">
                    <fo:block break-before="page"/>
                </xsl:if>
        ...
        <xsl:choose>
            ...
            <xsl:when test="not($noteImagePath = '')">
                <fo:table xsl:use-attribute-sets="note__table page.break">
                    <!--<xsl:if test="count(ancestor-or-self::*[contains(@class, ' topic/ul ')]) = 2">
        ...
        <xsl:choose>
                ...
                <fo:block xsl:use-attribute-sets="parLi.fchiP">
                    ...
                    <xsl:if test="@dir='ltr'">
                        <xsl:attribute name="text-align">
                            <xsl:value-of select="if (matches($localeStrAfter, '(SA|IR|IL|PK)') ) then
                                                  'end' else
                                                  'start'"/>
                        </xsl:attribute>
            ...
            <xsl:otherwise>
                <fo:block xsl:use-attribute-sets="p">
                    ...
                    <xsl:if test="matches(@outputclass, '^box\s?')">
                        <xsl:attribute name="border" select="'0.5pt solid black'" />
                        <xsl:attribute name="padding" select="'2.5mm'" />
                        <xsl:attribute name="start-indent" select="'3mm'" />
                        <xsl:attribute name="end-indent" select="'3mm'" />
                    </xsl:if>
    ...
    <xsl:template match="*" mode="inlineTextOptionalKeyref">
        ...
        <xsl:param name="contents" as="node()*">
            ...
            <xsl:if test="@outputclass = 'space4'">
                <xsl:attribute name="inline-progression-dimension" select="'45mm'" />
            </xsl:if>
        ...
        <xsl:choose>
            ...
            <xsl:when test="matches($localeStrAfter, '(SA|IR|IL|PK)') and
                            (@dir = 'ltr' or
                            @outputclass = 'no-trans' or
                            contains(., '+') or
                            matches(., '\d+\s*x\s*\d+'))">
                <fo:bidi-override direction="ltr" unicode-bidi="normal">
                    <fo:inline>
                        <xsl:sequence select="$copyAttributes/@*"/>
                        <xsl:call-template name="commonattributes"/>
                        <xsl:copy-of select="$contents"/>
            ...
            <xsl:otherwise>
                <xsl:choose>
                    <xsl:when test="matches(@outputclass, 'intuitive')">
                        <fo:block xsl:use-attribute-sets="inlineTblock">
                            <xsl:sequence select="$copyAttributes/@*"/>
                            <xsl:call-template name="commonattributes"/>
                            <xsl:copy-of select="$contents"/>
                        </fo:block>
    ...
    <xsl:template match="*[contains(@class,' topic/section ')]">
        <fo:block xsl:use-attribute-sets="section">
            ...
            <xsl:if test="preceding-sibling::node()[last()][self::processing-instruction('pagebreak')]">
                <fo:block break-before="page"/>
            </xsl:if>
        </fo:block>
    </xsl:template>
    ...
    <xsl:template match="*" mode="processTopic">
        <xsl:if test="node()[1][self::processing-instruction('pagebreak')]">
            <fo:block break-before="page"/>
        </xsl:if>
        ...
        <xsl:if test="node()[last()][self::processing-instruction('pagebreak')]">
    ...
    <xsl:template match="*[contains(@class,' topic/ph ')]">
        <xsl:apply-templates select="." mode="inlineTextOptionalKeyref">
            <xsl:with-param name="copyAttributes" as="element()">
                <wrapper xsl:use-attribute-sets="ph"/>
            </xsl:with-param>
        </xsl:apply-templates>
    </xsl:template>
    ...
    <xsl:template match="*[contains(@class, ' topic/tm ')]">
        ...
        <fo:inline xsl:use-attribute-sets="tm">
            ...
            <xsl:choose>
                ...
                <xsl:when test="matches(@tmtype, 'dita-use-conref-target')">
                    <fo:inline xsl:use-attribute-sets="tm__content__custom">
                        <xsl:choose>
                            <xsl:when test="@id = 'plus6'">
