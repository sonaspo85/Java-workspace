...
<xsl:stylesheet
    ...
    <xsl:attribute name="xml:lang">
      <xsl:choose>
          <xsl:when test="matches($localeStrAfter, '(HK|TW)')">
              <xsl:value-of select="'zh'" />
          </xsl:when>

          <xsl:when test="matches($localeStrAfter, 'SG')">
    ...
    <!-- <xsl:attribute name="axf:word-break">normal</xsl:attribute> -->

    <xsl:attribute name="axf:word-wrap">
      <xsl:choose>
        <xsl:when test="matches(substring-before($locale, '-'), 'th|lo|bn|km|zh')">break-word</xsl:when>
