  ...
  <xsl:template match="/" mode="create-page-masters">
    ...
    <xsl:if test="$generate-back-cover">
      <xsl:if test="$mirror-page-margins">
        <fo:simple-page-master master-name="back-cover-even" xsl:use-attribute-sets="simple-page-master">
          <fo:region-body xsl:use-attribute-sets="back-cover.even"/>
      ...
      <fo:simple-page-master master-name="back-cover-odd" xsl:use-attribute-sets="simple-page-master">
        <fo:region-body xsl:use-attribute-sets="back-cover.odd"/>
    ...
    <xsl:if test="$mirror-page-margins">
      <fo:simple-page-master master-name="toc-even" xsl:use-attribute-sets="simple-page-master">
        <fo:region-body xsl:use-attribute-sets="ast-toc-body.even"/>
    ...
    <fo:simple-page-master master-name="toc-odd" xsl:use-attribute-sets="simple-page-master">
      <fo:region-body xsl:use-attribute-sets="ast-toc-body.odd"/>
    ...
    <fo:simple-page-master master-name="toc-last" xsl:use-attribute-sets="simple-page-master">
      <fo:region-body xsl:use-attribute-sets="ast-toc-body.last"/>
      <fo:region-before region-name="even-toc-header" xsl:use-attribute-sets="region-before"/>
      <fo:region-after region-name="even-toc-footer" xsl:use-attribute-sets="region-after"/>
    </fo:simple-page-master>

    <fo:simple-page-master master-name="toc-first" xsl:use-attribute-sets="simple-page-master">
      <fo:region-body xsl:use-attribute-sets="ast-toc-body.first"/>
    ...
    <fo:simple-page-master master-name="body-first" xsl:use-attribute-sets="simple-page-master">
      <xsl:if test="matches($OSname, '-OS_upgrade')">
          <xsl:attribute name="background-color">cmyk(0%,0%,0%,10%)</xsl:attribute>
      </xsl:if>

      <xsl:choose>
          <xsl:when test="matches($OSname, '-OS_upgrade')">
              <fo:region-body xsl:use-attribute-sets="region-chapter.first"/>
          </xsl:when>
          <xsl:otherwise>
  ...
  <xsl:template name="generate-page-sequence-master">
    ...
    <fo:page-sequence-master master-name="{$master-name}">
      <fo:repeatable-page-master-alternatives>
        <xsl:if test="$first">
	        <xsl:choose>
	          <xsl:when test="$master-reference='toc'">
          		<fo:conditional-page-master-reference master-reference="{$master-reference}-first" page-position="first"/>
	          </xsl:when>
	          <xsl:otherwise>
          		<fo:conditional-page-master-reference master-reference="{$master-reference}-first" odd-or-even="any" page-position="first"/>
	          </xsl:otherwise>
	        </xsl:choose>
        </xsl:if>
        <xsl:if test="$last">
	        <xsl:choose>
	          <xsl:when test="$master-reference='toc'">
          		<fo:conditional-page-master-reference master-reference="{$master-reference}-last" page-position="last"/>
	          </xsl:when>
	          <xsl:otherwise>
          		<fo:conditional-page-master-reference master-reference="{$master-reference}-last" odd-or-even="any" page-position="last"/>
	          </xsl:otherwise>
	        </xsl:choose>
        </xsl:if>
        <xsl:choose>
