...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="fontFamily">
        ...
        <xsl:if test="matches($class, 'topic/ph ') and
            ...
            <xsl:choose>
                <xsl:when test="$cur[matches(@outputclass, 'small')]">
                    <xsl:call-template name="getVariable">
                        <xsl:with-param name="id" select="concat('outputclass=small)', ' FontFamily')" />
                    </xsl:call-template>
                </xsl:when>
    ...
    <xsl:template name="lineHeight">
        ...
        <xsl:if test="matches($class, 'topic/li ') and
            ...
            <xsl:variable name="ancesNote" select="$cur/ancestor::*[contains(@class, ' topic/note ')]" />
            ...
            <xsl:choose>
                <xsl:when test="$ancesNote[matches(@outputclass, 'roundbox')]">
                    <xsl:call-template name="getVariable">
                        <xsl:with-param name="id" select="concat('outputclass=roundbox)', ' topic/li', ' LineHeight')" />
                    </xsl:call-template>
                </xsl:when>

                <xsl:when test="$ancesNote or $ancesTable">
                    <xsl:call-template name="getVariable">
                        <xsl:with-param name="id" select="concat('ancesNote)', ' topic/li', ' LineHeight')" />
                    </xsl:call-template>
                </xsl:when>
